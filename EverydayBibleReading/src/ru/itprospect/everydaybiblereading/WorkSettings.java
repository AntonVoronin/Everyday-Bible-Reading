...
public class WorkSettings {
	...
	public WorkSettings(GregorianCalendar date, String conf) {
		...
		if (conf.equals(CONF_ORTHODOXY)) {
			if (date.compareTo(new GregorianCalendar(2013, 2, 1))>=0 && date.compareTo(new GregorianCalendar(2017, 9, 31))<=0) {
				fileFindSuccess = true;
			}
		} 
		else if (conf.equals(CONF_CATHOLIC)) {
			if (date.compareTo(new GregorianCalendar(2013, 11, 1))>=0 && date.compareTo(new GregorianCalendar(2017, 9, 31))<=0) {
				fileFindSuccess = true;
			}			
		};
